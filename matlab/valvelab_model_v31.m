    ...
    methods (Static)
        function c = defaults(valve)
            ...
            c = struct('valveId',valve,'mode','manual','manual',65,'setpoint',60, ...
                ...
                'inlet',2,'outlet',1,'fault','none','leakage',0.05,'sensorBias',15,'sensorTau',0.2);
        end
        ...
        function names = signalNames()
            names = [valvelab_model_v31.stateNames(), {'setpoint','inlet','outlet','appliedRevision'}];
        end
        function validate(c)
            d = valvelab_model_v31.defaults();
            ...
            ranges = {'manual',0,100;'setpoint',0,150;'kp',0,5;'ki',0,3;'kd',0,1; ...
                ...
                'leakage',0.01,0.15;'sensorBias',-30,30;'failPosition',0,100;'sensorTau',0.05,1};
            ...
            assert(any(abs(c.sensorTau-[.05 .1 .2 .5 1])<1e-10),'ValveLab:Config','Sensor tau must use one of the five teaching presets.');
        end
        function v = encode(c)
            ...
            v=[find(strcmp(c.valveId,{'ball','butterfly','gate','globe','control'})), ...
                ...
                c.inlet,c.outlet,find(strcmp(c.fault,{'none','stuck','supply','leak','sensor'}))-1,c.leakage,c.sensorBias,c.sensorTau];
        end
        function c = decode(v)
            ...
            c=struct('valveId',valves{v(1)},'mode',modes{v(2)+1},'manual',v(3),'setpoint',v(4), ...
                ...
                'fault',faults{v(14)+1},'leakage',v(15),'sensorBias',v(16),'sensorTau',v(17));
        end
        function v = pack(s)
            names=valvelab_model_v31.stateNames(); v=zeros(1,numel(names));
            for n=1:numel(names), v(n)=s.(names{n}); end
        end
        function s = unpack(v)
            s=cell2struct(num2cell(v(:)),valvelab_model_v31.stateNames(),1);
        ...
        function s = initial(c)
            valvelab_model_v31.validate(c); [q,leak]=valvelab_model_v31.flow(c,65);
            ...
            s=valvelab_model_v31.unpack([0 65 command q pv leak integral 0 pv p integral 0 c.setpoint-pv 0 0 1]);
        end
        function s = advance(s,previous,c,dt)
            ...
            [q,leak]=valvelab_model_v31.flow(c,opening);
            sensor=max(0,q+strcmp(c.fault,'sensor')*c.sensorBias);
            pv=s.measuredFlow+(sensor-s.measuredFlow)*(1-exp(-dt/c.sensorTau));
            saturated=raw<0 || raw>100 || (command>=99.99 && err>0.1) || (command<=0.01 && err< -0.1);
            s=valvelab_model_v31.unpack([s.time+dt opening command q pv leak integral derivative ...
